...
const envSchema = z.object({
  ...
  // Monitoring & Logging
  SENTRY_DSN: z.union([z.string().url('Invalid SENTRY_DSN format'), z.literal('')]).optional(),
...
const productionSchema = envSchema
  ...
  .refine(
    (data) => {
      if (data.NODE_ENV === 'production') {
        // Ensure monitoring is configured in production (must be a valid URL, not empty)
        return data.SENTRY_DSN && data.SENTRY_DSN !== '';
      }
      return true;
    },
    {
